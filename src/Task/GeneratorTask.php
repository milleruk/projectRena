...
class GeneratorTask extends Command
{
    ...
    {
        $this->setName('generator')->setDescription('Generates Controllers / Models / Libs / Tasks / Cronjobs / Resque Queues')->addOption("controller", "c", InputOption::VALUE_NONE, "Create Controller")->addOption("model", "m", InputOption::VALUE_NONE, "Create Model")->addOption("libs", "l", InputOption::VALUE_NONE, "Create Lib")->addOption("task", "t", InputOption::VALUE_NONE, "Create Task")->addOption("cronjob", "j", InputOption::VALUE_NONE, "Create Cronjob")->addOption("resque", "r", InputOption::VALUE_NONE, "Create Resque Queue");
    }
...
EOF;
    ...
    private function libs($name, $output)
    {
        ...
        // Create lib
        $class = new PhpClass();
        $class->setQualifiedName("ProjectRena\\Lib\\{$name}")->setDescription($this->descr)->setMethod(PhpMethod::create("__construct")->addParameter(PhpParameter::create("app")->setType("RenaApp"))->setBody("\$this->app = \$app;\n\$this->db = \$app->Db;\n\$this->config = \$app->baseConfig;\n\$this->cache = \$app->Cache;\n\$this->curl = \$app->cURL;\n\$this->statsd = \$app->StatsD;\n\$this->log = \$app->Logging;"))->setProperty(PhpProperty::create("app")->setVisibility("private")->setDescription("The Slim Application"))->setProperty(PhpProperty::create("db")->setVisibility("private")->setDescription("The Database"))->setProperty(PhpProperty::create("config")->setVisibility("private")->setDescription("The baseConfig (config/config.php)"))->setProperty(PhpProperty::create("cache")->setVisibility("private")->setDescription("The Cache"))->setProperty(PhpProperty::create("curl")->setVisibility("private")->setDescription("cURL interface (getData / setData)"))->setProperty(PhpProperty::create("statsd")->setVisibility("private")->setDescription("StatsD for tracking stats"))->setProperty(PhpProperty::create("log")->setVisibility("private")->setDescription("The logger, outputs to logs/app.log"))->declareUse('ProjectRena\RenaApp');
    ...
    private function task($name, $output)
    {
        ...
        $class = new PhpClass();
        $class->setQualifiedName("ProjectRena\\Task\\{$name}Task")->setParentClassName("Command")->setDescription($this->descr)->setMethod(PhpMethod::create("configure")->setVisibility("protected")->setBody("\$this->setName('$name')->setDescription('" . $this->descr["description"] . "');"))->setMethod(PhpMethod::create("execute")->setVisibility("protected")->addParameter(PhpParameter::create("input")->setType("InputInterface"))->addParameter(PhpParameter::create("output")->setType("OutputInterface"))->setBody("//Init rena\n\$app = RenaApp::getInstance();"))->declareUses('ProjectRena\RenaApp', 'Cilex\Command\Command', 'ProjectRena\Lib', 'Symfony\Component\Console\Input\InputInterface', 'Symfony\Component\Console\Output\OutputInterface');
    ...
    private function cronjob($name, $output)
    {
        ...
        $class = new PhpClass();
        $class->setQualifiedName("ProjectRena\\Task\\Cronjobs\\{$name}Cronjob")->setDescription($this->descr)->setMethod(PhpMethod::create("getRunTimes")->setVisibility("public")->setStatic(true)->setDescription("Defines how often the cronjob runs, every 1 second, every 60 seconds, every 86400 seconds, etc.")->setBody("return 0; // Never runs"))->setMethod(PhpMethod::create("execute")->setVisibility("public")->setStatic(true)->setDescription("Executes the cronjob task")->addParameter(PhpParameter::create("pid"))->addParameter(PhpParameter::create("md5"))->addParameter(PhpParameter::create("app")->setType("RenaApp"))->setBody("exit(); // Keep this at the bottom, to make sure the fork exits"))->declareUse('ProjectRena\RenaApp');
    ...
    private function resque($name, $output)
    {
        ...
        $class = new PhpClass();
        $class->setQualifiedName("ProjectRena\\Task\\Resque\\{$name}")->setProperty(PhpProperty::create("app")->setVisibility("private")->setDescription("The Slim Application"))->setDescription($this->descr)->setMethod(PhpMethod::create("setUp")->setVisibility("public")->setDescription("Sets up the task (Setup \$this->crap and such here)")->setBody("\$this->app = \\ProjectRena\\RenaApp::getInstance();"))->setMethod(PhpMethod::create("perform")->setVisibility("public")->setDescription("Performs the task, can access all \$this->crap setup in setUp)"))->setMethod(PhpMethod::create("tearDown")->setVisibility("public")->setDescription("Tears the task down, unset \$this->crap and such")->setBody("\$this->app = null;"));
